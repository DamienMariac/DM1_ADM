...
\]
Avec, $\overline{x^j}$ représente la moyenne globale des $x^j$, $\overline{x^j}^q$ représente la moyenne pondérée des $x^j$ pour le type forestier $q$.\\
...
\end{align*}
Ici, $\left\lVert \Pi_Y x^j \right\rVert^2 _W$ s'interprete statistiquement comme
...
\begin{align*}
    ...
    &= R^2(x^j\mid Y )
\end{align*}
Donc $tr(\Pi_{x^{j}}\Pi_Y)$ représente le $R^2$ d'une densité de population sachant son type forestier. C'est a dire que nous 
...
\section{Conclusion}
    On remarque que les deux méthodes donnent les memes resultats, bien que la méthode de la deuxieme partie est plus efficace  par la simplicité des formules.
\\
\\
    Les resultats semblent cohérent, car la disparité des arbres est fortement impacté par leur demande en ressource. Le type géologique est donc naturellement un indice important dans la croissance de ces derniers.

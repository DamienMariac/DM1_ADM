...
\[
(x_i^j)_{1 \leq j \leq 27} = \frac{x_j^i - \bar{x}_j}{\sigma_j}
\]
...
(Une matrice $X \in \mathcal{M}_{1000,27}(\mathbb{R})$).
...
On s'interesse maintenant à l'interpretation des resultats trouvé. Nous calculons alors le $R^2$ de chaque variable densité avec
 $R^2 = \frac{variance_{inter} - types forestiers}{variance totale}$.
